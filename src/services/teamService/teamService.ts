...
export const fetchTeams = (abortController?: AbortController): Promise<FetchTeamsResponse> => fetch("/teams.json",{
...
export const getTeamsAverage = (teams: Team[]): TeamValues => {
    const teamValuesSum = teams.reduce<TeamValues>((values, { name, ...currentTeamValues }) => {
        const sum = { ...values }

        Object.keys(currentTeamValues).forEach((key) => {
            const teamValuesKeys = (key as keyof TeamValues)
            sum[teamValuesKeys] = sum[teamValuesKeys] + currentTeamValues[teamValuesKeys]
        })
    ...
    Object.keys(teamValuesSum).forEach((key) => {
        const teamValuesKeys = (key as keyof TeamValues)
        teamValuesAverage[teamValuesKeys] = teamValuesSum[teamValuesKeys] / teams.length
    })
